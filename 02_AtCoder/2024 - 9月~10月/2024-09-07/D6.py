...
class UnionFind :
    ...
    def find(self, x) :
        ...
        if self.parents[x] < 0 : return x #0未満 (親が存在しない場合、入力値xは全ての親であるということ)
            ...
            #print("現在いるnodeは全ての親ではありません。調査を行います。")
            #print(f"xの値{x}")
    ...
    def get_break_points(self, x) :
        ...
        - 注意?: 常に参照するアイテムは親全ての親オブジェクトであるべきである。なので入力値も必ずx = find(x)をしなければならない?
        ...
        results = [] #位置を格納する変数
        # 左に調査
        #print(f"調査を開始します。現在着目点はx{x}")
        #print(f"この行(列)における、parents{self.parents}")
        #print(f"現在のself.L[x] = {self.L[x]}")
        #print(f"現在のself.L[x-1] = {self.L[x-1]}")
        #print(f"現在のself.parents[self.L[x]] {self.parents[self.L[x]]}")
        #print(f"現在のself.parents[self.L[x-1]{self.parents[self.L[x-1]]}")
